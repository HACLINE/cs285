...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            assert self.num_critic_networks == 2, "Double-Q requires two critic networks"
            next_qs = torch.stack([next_qs[1], next_qs[0]], dim = 0)
        elif self.target_critic_backup_type == "min":
            next_qs = torch.min(next_qs, dim = 0).values.expand(self.num_critic_networks, -1)
        elif self.target_critic_backup_type == "mean":
            next_qs = next_qs.mean(dim = 0).expand(self.num_critic_networks, -1)
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_qs = next_qs.clone()
                next_qs += self.temperature * next_action_entropy

            # Compute the target Q-value
            target_values: torch.Tensor = reward.unsqueeze(0).expand(self.num_critic_networks, -1) + self.discount * (~done) * next_qs
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            obs_expanded = obs.unsqueeze(0).expand(self.num_actor_samples, -1, -1)  # (num_actor_samples, batch_size, obs_dim)
            obs_expanded = obs_expanded.reshape(-1, obs.shape[1])  # (num_actor_samples * batch_size, obs_dim)
            action_reshaped = action.reshape(-1, self.action_dim)  # (num_actor_samples * batch_size, action_dim)
            q_values = self.critic(obs_expanded, action_reshaped)  # (num_critic_networks, num_actor_samples * batch_size)
            advantage = q_values.mean(dim = 0) # (num_actor_samples * batch_size,)

        log_probs = action_distribution.log_prob(action).reshape(self.num_actor_samples * batch_size, )
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        q_values = torch.mean(q_values, axis = 0) # (batch_size,)
