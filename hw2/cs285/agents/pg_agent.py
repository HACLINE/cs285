...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        actions = np.concatenate(actions, axis = 0).reshape(-1, *actions[0].shape[1:])
        rewards = np.concatenate(rewards, axis = 0).reshape(-1, *rewards[0].shape[1:])
        terminals = np.concatenate(terminals, axis = 0).reshape(-1, *terminals[0].shape[1:])
        q_values = np.concatenate(q_values, axis = 0).reshape(-1, *q_values[0].shape[1:])
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    if terminals[i]:
                        ...
                        advantages[i] = self.gamma * self.gae_lambda * advantages[i + 1] + rewards[i] + self.gamma * values[i + 1] - values[i]
